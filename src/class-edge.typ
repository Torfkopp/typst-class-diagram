...
#import "edge.typ": interpret-edge-args, normalize-label-pos, interpret-marks-arg
...
#let get_arrow(arrow, direction, mark-size) = {
  ...
  let arrow = if direction == right {(none, arrow-mark)} else if direction == left {(arrow-mark, none)} else {(arrow-mark, arrow-mark)}
